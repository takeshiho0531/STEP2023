def get_digits(line: str, start_idx: int) -> dict:
    ...
    idx = start_idx
    digit_list = []
    while ((idx<len(line)) and (line[idx] in digits)):
...
def digits_to_number(digit_list):
    ...
    while ((idx<len(digit_list)) and (digit_list[idx] != ".")):
        number = number * 10 + int(digit_list[idx])
        idx += 1
        ...
        if idx==len(digit_list):
            ...
            print("decimal_point_idx", decimal_point_idx)
            idx += 1
            while idx < len(digit_list):
                number += int(digit_list[idx]) * (10 ** (decimal_point_idx-idx))
                idx += 1
            else:
                print("return number", number)
                return number
